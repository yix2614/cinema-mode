  ...
  return (
    <div className={engagementPanelClasses.container(className)} style={engagementPanelStyles.container(isSmall)}>
      ...
      >
        ...
        />
        <ActionButton 
          icon={<Icons.Comment fill={iconColor} color={iconColor} stroke="transparent" />} 
          label={data.comments} 
        ...
        />
        <ActionButton 
          icon={<Icons.Bookmark fill={iconColor} color={iconColor} stroke="transparent" />} 
          label={data.saves} 
          labelClassName={'mt-0'}
          labelStyle={{ lineHeight: '16px' }}
          size={btnSize}
        ...
        {!hideShareAndMusic && (
          <>
            <ActionButton 
              icon={<Icons.Share fill={iconColor} color={iconColor} stroke="transparent" />} 
              label={data.shares} 
              labelClassName={'mt-0'}
              labelStyle={{ lineHeight: '16px' }}
              size={btnSize}
...
const ActionButton: React.FC<ActionButtonProps> = ({ icon, label, onClick, labelClassName, labelStyle, size = 48, iconSize = 20 }) => (
  <div className={`${engagementPanelClasses.actionButton.wrapper} tux-button-border-fix`}>
      <div
        className="flex justify-center items-center rounded-full transition-all duration-200 active:scale-90 isolate overflow-hidden relative tux-button-border-fix"
        style={{...engagementPanelStyles.actionButton.button(size), padding: 0}}
      >
      <TUXIconButton
        size={size}
        icon={React.isValidElement(icon) ? React.cloneElement(icon as React.ReactElement<any>, { width: iconSize, height: iconSize, color: 'var(--tux-v2-color-ui-shape-text-1-on-primary)', fill: 'var(--tux-v2-color-ui-shape-text-1-on-primary)', stroke: 'transparent' }) : icon}
        onClick={onClick}
